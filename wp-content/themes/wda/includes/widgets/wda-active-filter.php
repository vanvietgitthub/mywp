...
class WDA_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = __( 'Wda Active Product Filters', 'woocommerce' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
			...
			if ( intval($min_price) === 1 && intval($max_price) > 1 ) {
				$link = remove_query_arg( array('min_price', 'max_price'), $base_link );
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( '< %s', 'woocommerce' ), wc_price( $max_price ) ) . '</a></li>';
			}elseif ( intval($min_price) > 1 && intval($max_price) === 1 ) {
				$link = remove_query_arg( array('min_price', 'max_price'), $base_link );
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( '> %s', 'woocommerce' ), wc_price( $min_price ) ) . '</a></li>';
			}elseif ( intval($min_price) > 1 && intval($max_price) > 1 && intval($min_price) < intval($max_price) ) {
				$link = remove_query_arg( array('min_price', 'max_price'), $base_link );
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . $min_price . ' - ' . $max_price . '</a></li>';
			}elseif ( intval($min_price) > 1 && intval($max_price) > 1 && intval($min_price) >= intval($max_price) ) {
				$link = remove_query_arg( array('min_price', 'max_price'), $base_link );
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'woocommerce' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( '%s', 'woocommerce' ), wc_price( $min_price ) ) . '</a></li>';
			}
...
function wda_register_widget_filter_nav() {
	register_widget( 'WDA_Widget_Layered_Nav_Filters' );
}
add_action( 'widgets_init', 'wda_register_widget_filter_nav' );